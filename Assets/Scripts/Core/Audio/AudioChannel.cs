...
public class AudioChannel
{
    ...
    public AudioTrack PlayTrack(AudioClip clip,bool loop,float startingVolum,float volumeCap,float pitch, string filePath)
    {
        ...
        AudioTrack track = new AudioTrack(clip,loop,startingVolum,volumeCap,pitch ,this,AudioManager.instance.musicMixer);
        track.Play();
    ...
    private IEnumerator VolumeLeveling()
    {
        while ((activeTrack != null && (tracks.Count > 1 || activeTrack.volume != activeTrack.volumeCap)) || (activeTrack == null && tracks.Count > 0))
        {
            for(int i=tracks.Count-1;i>=0;i--)
            {
                ...
                track.volume = Mathf.MoveTowards(track.volume, targetVol, AudioManager.TRACK_TRANSITIONN_SPEED * Time.deltaTime);
